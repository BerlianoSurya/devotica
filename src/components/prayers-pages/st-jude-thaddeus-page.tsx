...
interface StJudeThaddeusPrayerStep {
  id: number;
  title: string;
  content: string;
  count?: number;
...
export function StJudeThaddeusPrayer() {
  const t = useTranslations("prayers.st-jude-thaddeus");
  ...
  const handleClick = () => {
    startTransition(async () => {
      const res = await trackUserPrayer(`st-jude-thaddeus`);
      if (!res.success) {
        if (res.redirectTo) {
  ...
  const generateSteps = (): StJudeThaddeusPrayerStep[] => {
    const steps: StJudeThaddeusPrayerStep[] = [];
    let id = 0;
      ...
      <main className="container mx-auto px-4 py-6 ">
        <div className="border rounded-lg p-4 sm:p-6 bg-card">
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              <div className="h-78 md:h-90 lg:h-120 overflow-y-auto w-full">
