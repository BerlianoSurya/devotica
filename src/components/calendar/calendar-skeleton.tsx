    ...
    <div className="lg:flex lg:h-full lg:flex-col bg-card border border-border rounded-xl overflow-hidden">
      ...
      <header className="flex items-center justify-between border-b border-border px-6 py-4 lg:flex-none">
        ...
        <div className="flex items-center">
          ...
          <Button variant="ghost" size="sm" disabled className="ml-6 md:hidden">
        ...
        <div className="flex bg-muted text-xs text-muted-foreground lg:flex-auto">
          <div className="hidden w-full lg:grid lg:grid-cols-7 lg:grid-rows-6 lg:gap-px">
            {Array.from({ length: 42 }).map((_, i) => (
              <div
                key={i}
                className="bg-background relative px-3 py-2 min-h-[120px] animate-pulse"
              >
                <Skeleton className="h-6 w-6 rounded-full" />
                <div className="mt-2 space-y-1">
                  {Math.random() > 0.6 && <Skeleton className="h-3 w-full" />}
                  {Math.random() > 0.7 && <Skeleton className="h-3 w-3/4" />}
                  {Math.random() > 0.8 && <Skeleton className="h-3 w-1/2" />}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="isolate grid w-full grid-cols-7 grid-rows-6 gap-px lg:hidden">
            {Array.from({ length: 42 }).map((_, i) => (
              <div
                key={i}
                className="bg-background flex h-14 flex-col px-3 py-2 animate-pulse"
              >
                <Skeleton className="h-4 w-4 ml-auto rounded-full" />
                <div className="mt-auto flex flex-wrap-reverse -mx-0.5">
                  {Math.random() > 0.7 && (
                    <div className="mx-0.5 mb-1 h-1.5 w-1.5 rounded-full bg-muted-foreground animate-pulse" />
                  )}
                  {Math.random() > 0.8 && (
                    <div className="mx-0.5 mb-1 h-1.5 w-1.5 rounded-full bg-muted-foreground animate-pulse" />
                  )}
